...
export class ApiService {
  ...
  OnlinePaymentChallan(transaction_id,payment_amount,payment_status,application_id,payment_date_time,enrollment_no,user_id){
    try{
      return this.httpClient.post(`${this.baseUrl}/api/payment/OnlinePaymentChallan`,{user_id:user_id,application_no:enrollment_no,payment_amount:payment_amount,transaction_id:transaction_id,date_time:payment_date_time,status_payment:payment_status,application_id:application_id});
    }catch(error) {
...
//Cart 
  ...
  editPreferences(course_id,edit_pref){
    try{
      return  this.httpClient.post(`${this.baseUrl}/editPreferences`,{"course_id" : course_id, "edit_pref" : edit_pref});
    }catch(error){
      this.handleError("editPreferences : "+JSON.stringify(error));
    }
  }
  ...
  getTheme()
  {
    ...
    .subscribe((token: NbAuthJWTToken) => {
      if (token.isValid()) {
        if(token.getPayload()['theme'] == ""){
          this.themeService.changeTheme('default');
        }else{
          this.themeService.changeTheme(token.getPayload()['theme']);
        }
    }
